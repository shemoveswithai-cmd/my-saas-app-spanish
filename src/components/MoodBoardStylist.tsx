...
const ImageUploader: React.FC<{
    ...
    return (
        <div className="flex flex-col items-center flex-1">
            <h4 className="text-sm font-bold mb-2 text-negro-fondo opacity-80 uppercase">{title}</h4>
            <div onClick={() => inputRef.current?.click()} className="w-full aspect-[4/5] bg-white/50 rounded-lg flex items-center justify-center cursor-pointer border-2 border-dashed border-rosa-principal/50 hover:border-rosa-principal transition-colors p-1 relative overflow-hidden">
                {image ? (
                    <img src={image.preview} className="w-full h-full object-cover rounded-md" />
                ) : (
                    <div className="text-center text-negro-fondo opacity-40">
    ...
    return (
        <div className="flex flex-col h-full bg-rosa-claro rounded-lg shadow-xl p-4 md:p-6 space-y-4 overflow-y-auto">
            <div className="bg-white rounded-[1.5rem] shadow-sm p-5 md:p-6 text-center mb-6 border border-rosa-principal/5 max-w-3xl mx-auto">
                <h1 className="text-xl md:text-2xl font-bold text-negro-fondo mb-2 uppercase tracking-tight">Mood Board IA</h1>
            ...
            {generatedImages.length > 0 && <div className="flex justify-center mb-4"><button onClick={() => {setGeneratedImages([]); setMoodboard(null); setPersonA(null); setPersonB(null);}} className="text-sm py-2 px-4 rounded-lg bg-white/80 hover:bg-white text-negro-fondo font-semibold shadow-sm">Empezar de nuevo</button></div>}
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                    <Step number={1} title="Elegir Modo">
                        <div className="flex bg-white/50 rounded-full p-1">
                            <button onClick={() => setMode('one')} className={`w-1/2 text-center rounded-full py-2 text-sm font-bold transition-all ${mode === 'one' ? 'bg-rosa-principal text-negro-fondo' : 'text-negro-fondo opacity-60'}`}>1 Persona</button>
                            <button onClick={() => setMode('two')} className={`w-1/2 text-center rounded-full py-2 text-sm font-bold transition-all ${mode === 'two' ? 'bg-rosa-principal text-negro-fondo' : 'text-negro-fondo opacity-60'}`}>2 Personas (Fusión)</button>
                        </div>
                    </Step>

                    <Step number={2} title="Subir Inspiración">
                        <p className="text-xs text-negro-fondo opacity-80 mb-3 italic">Sube tu moodboard (el estilo) y la persona que quieres estilizar.</p>
                    ...
                    <Step number={3} title="Personalizar (Opcional)">
                        <div className="space-y-3">
                            <div>
                                <label className="text-[10px] font-bold text-negro-fondo opacity-60 uppercase block mb-1">Modificador de Prompt Personalizado</label>
                                <textarea 
                                    value={customPrompt} 
                                    onChange={(e) => setCustomPrompt(e.target.value)}
                                    placeholder="ej: usando gafas de sol, en una ciudad futurista, sosteniendo un café..."
                                    className="w-full bg-white/50 border border-rosa-principal/20 rounded-lg p-2 text-sm focus:outline-none focus:border-rosa-principal min-h-[60px] resize-none"
                                />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-negro-fondo opacity-60 uppercase block mb-1">Relación de Aspecto</label>
                                <div className="grid grid-cols-5 gap-1">
                                    {['1:1', '3:4', '4:3', '9:16', '16:9'].map((ratio) => (
                                        <button
                                            key={ratio}
                                            onClick={() => setAspectRatio(ratio)}
                                            className={`py-1 text-[10px] font-bold rounded-md border transition-all ${aspectRatio === ratio ? 'bg-rosa-principal border-rosa-principal text-negro-fondo' : 'bg-white/30 border-rosa-principal/20 text-negro-fondo opacity-60'}`}
                                        >
                                            {ratio}
                                        </button>
                                    ))}
                    ...
                    <button onClick={() => handleGenerate()} disabled={isLoading || cooldown > 0 || !moodboard || !personA} className="w-full bg-rosa-principal text-negro-fondo font-bold py-4 rounded-lg hover:bg-white disabled:bg-rosa-principal/50 disabled:cursor-not-allowed shadow-md text-lg transition-all">
                        {cooldown > 0 ? (
                            <span className="text-sm uppercase tracking-widest">Modelo ocupado — reintentando en {cooldown}s</span>
                        ) : isLoading ? (
                            <div className="flex flex-col items-center">
                                <Spinner className="mx-auto text-negro-fondo" />
                ...
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-4 rounded-lg flex flex-col min-h-[400px]">
                    <h3 className="text-lg font-bold text-center text-negro-fondo mb-4 border-b border-rosa-principal/20 pb-2">Tus Avatares Estilizados</h3>
                            ...
                            <div key={i} className="relative aspect-[4/5] bg-white/40 rounded-lg flex items-center justify-center overflow-hidden">
                                {isLoading && i >= generatedImages.length ? (
                                    <div className="text-center px-2">
                                        <Spinner className="w-8 h-8 text-rosa-principal mb-2 mx-auto" />
                                        <p className="text-[9px] font-black text-negro-fondo opacity-40 uppercase tracking-widest leading-tight">{loadingMessage}</p>
                                    </div>
                                ) : generatedImages[i] ? (
                                ...
                                ) : <span className="opacity-20 text-4xl font-bold text-negro-fondo">{i + 1}</span>}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="space-y-6 mt-8">
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    <h2 className="text-2xl font-bold text-negro-fondo mb-4 text-center">Introducción</h2>
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isIntroExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[100px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsIntroExpanded(!isIntroExpanded)} className="mt-2 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
                ...
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    <h2 className="text-2xl font-bold text-negro-fondo mb-4 text-center">Cómo funciona</h2>
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isHowItWorksExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[100px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsHowItWorksExpanded(!isHowItWorksExpanded)} className="mt-2 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
                ...
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    <h2 className="text-2xl font-bold text-negro-fondo mb-4 text-center">Estrategia de Ventas y Marketing</h2>
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isMarketingExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[100px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsMarketingExpanded(!isMarketingExpanded)} className="mt-2 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
