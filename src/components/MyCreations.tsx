    ...
    return (
        <div className="flex flex-col h-full bg-smw-pink-light rounded-lg shadow-xl p-4 md:p-6 space-y-4">
            <div className="flex-shrink-0">
                <h2 className="text-2xl font-bold text-smw-gray-dark">Mis Creaciones</h2>
                <p className="text-sm text-smw-gray-dark opacity-80">Una galería de todas tus imágenes generadas por IA.</p>
            </div>
            
            <div className="flex-shrink-0 bg-white/60 backdrop-blur-sm shadow-md p-3 rounded-lg flex items-center gap-3 text-sm text-smw-gray-dark opacity-80">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 flex-shrink-0 text-smw-pink" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pb-4 border-b border-smw-pink/20 flex-shrink-0">
                <div className="flex items-center bg-white/60 rounded-full p-1 w-full sm:w-auto justify-center">
                    {['Todo', 'Favoritos'].map((f: any) => (
                        <button key={f} onClick={() => setActiveFilter(f)} className={`px-4 py-1 text-sm rounded-full transition-colors flex-1 sm:flex-none ${activeFilter === f ? 'bg-smw-pink text-smw-gray-dark font-bold' : 'text-smw-gray-dark opacity-80 hover:bg-white'}`}>{f}</button>
                    ))}
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto justify-center">
                    <button onClick={handleDownload} className="flex-1 sm:flex-none bg-white/80 text-smw-gray-dark font-bold py-2 px-3 sm:px-4 rounded-lg hover:bg-white disabled:opacity-50 text-xs sm:text-sm">Descargar</button>
                    <button onClick={toggleSelectMode} className="flex-1 sm:flex-none bg-smw-pink text-smw-gray-dark font-bold py-2 px-3 sm:px-4 rounded-lg hover:bg-white text-xs sm:text-sm">{isSelectMode ? 'Cancelar' : 'Seleccionar'}</button>
                    {creations.length > 0 && (
                        <button 
            ...
            <div className="flex-1 overflow-y-auto pr-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-20">
                        {displayedCreations.map((creation) => (
                            <div key={creation.id} className="relative group rounded-xl overflow-hidden shadow-lg border border-white/50 bg-white">
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                        <div className="flex items-center gap-2 p-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/10 shadow-2xl">
                                            <button 
                                                ...
                                                className="p-1 text-white hover:text-smw-pink transition-colors active:scale-90"
                                                title={favorites.has(creation.id) ? "Quitar de favoritos" : "Favorito"}
                                            >
                                                {favorites.has(creation.id) ? (
                                                    <svg className="h-4 w-4 text-smw-pink" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" /></svg>
                                                ) : (
                                ...
                                {isSelectMode && (
                                    <div 
                                        className={`absolute inset-0 flex items-center justify-center transition-colors pointer-events-none ${selectedImageIds.has(creation.id) ? 'bg-black/40 border-4 border-smw-pink' : 'bg-transparent'}`}
                                    >
                ...
                ) : <div className="flex flex-col items-center justify-center h-full text-smw-gray-dark opacity-60"><p>Aún no hay creaciones aquí.</p></div>}
            </div>

            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[5000] p-4">
                    <div className="bg-smw-gray-dark p-6 rounded-lg border border-smw-pink max-w-sm text-center">
                        <h3 className="text-xl font-bold mb-4">¿Eliminar creación?</h3>
                        <div className="flex justify-center gap-4">
                            <button onClick={() => setShowDeleteConfirm(null)} className="py-2 px-6 rounded-lg bg-smw-gray-light font-bold">Cancelar</button>
            ...
            {showDeleteAllConfirm && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[5000] p-4">
                    <div className="bg-smw-gray-dark p-6 rounded-lg border border-red-600 max-w-sm text-center">
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <button onClick={() => setShowDeleteAllConfirm(false)} className="w-full py-2 rounded-lg bg-smw-gray-light font-bold">Cancelar</button>
                        </div>
                    </div>
                </div>
            )}
