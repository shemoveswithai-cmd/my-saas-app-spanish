...
const ColorSwatch: React.FC<{ hex: string, isSelected: boolean, onClick: () => void }> = ({ hex, isSelected, onClick }) => (
    <button 
        onClick={onClick}
        className={`w-10 h-10 rounded-full border-2 transition-all transform hover:scale-105 ${isSelected ? 'border-rosa-principal ring-2 ring-rosa-principal ring-offset-2 ring-offset-white' : 'border-gray-200'}`}
...
const PillButton: React.FC<{ label: string, isSelected: boolean, onClick: () => void }> = ({ label, isSelected, onClick }) => (
    <button 
        onClick={onClick}
        className={`px-6 py-2 rounded-full text-sm font-semibold transition-all ${isSelected ? 'bg-rosa-principal text-negro-fondo shadow-md' : 'bg-white text-negro-fondo border border-gray-100 hover:bg-gray-50'}`}
    >
        {label}
    </button>
);
    ...
    return (
        <div className="flex flex-col bg-rosa-claro rounded-lg shadow-xl p-4 md:p-6 space-y-4 h-full overflow-y-auto">
            <div className="bg-white rounded-[1.5rem] shadow-sm p-5 md:p-6 text-center mb-6 border border-rosa-principal/5 max-w-3xl mx-auto">
                <h1 className="text-xl md:text-2xl font-bold text-negro-fondo mb-2 uppercase tracking-tight">Estudio de Maquillaje IA</h1>
            ...
            {generatedImages.length > 0 && <div className="flex justify-center mb-4"><button onClick={() => {setGeneratedImages([]); setPersonImage(null);}} className="text-sm py-2 px-4 rounded-lg bg-white/80 hover:bg-white text-negro-fondo font-semibold shadow-sm">Empezar de nuevo</button></div>}

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                    <Step number={1} title="Sube tu foto">
                        <p className="text-sm text-negro-fondo opacity-80 mb-3">Sube un selfie o retrato claro. Para obtener los mejores resultados, asegúrate de que tu rostro esté bien iluminado y mire directamente a la cámara.</p>
                        <div 
                            onClick={() => personInputRef.current?.click()} 
                            className="aspect-square max-w-[300px] mx-auto bg-white/60 rounded-lg flex items-center justify-center border-2 border-dashed border-rosa-principal/50 cursor-pointer hover:bg-white transition-colors p-4"
                        >
                            {personImage ? (
                                <img src={personImage.preview} className="max-h-full max-w-full rounded-md shadow-sm" alt="Vista previa" />
                            ) : (
                                <div className="text-center space-y-2 text-negro-fondo opacity-60">
                    ...
                    <Step number={2} title="Elige un look">
                        <div className="space-y-6">
                            ...
                            <div className="pt-4 border-t border-rosa-principal/20 space-y-6">
                                <h4 className="text-sm font-bold text-negro-fondo opacity-60 uppercase tracking-wider">Personalizar</h4>
                                
                                <div className="space-y-3">
                                    <p className="text-sm font-semibold text-negro-fondo">Labios</p>
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm font-semibold text-negro-fondo">Rubor</p>
                                    <div className="flex flex-wrap gap-3">
                                        {blushColors.map(c => (
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm font-semibold text-negro-fondo">Sombra de ojos</p>
                                    <div className="flex flex-wrap gap-3">
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm font-semibold text-negro-fondo">Delineador de ojos</p>
                                    <div className="flex flex-wrap gap-2">
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm font-semibold text-negro-fondo">Pestañas</p>
                                    <div className="flex flex-wrap gap-2">
                                        {lashStyles.map(l => (
                        ...
                        <button 
                            ...
                            className="w-full mt-8 bg-rosa-principal text-negro-fondo font-bold py-4 rounded-lg hover:bg-white disabled:bg-rosa-principal/50 disabled:cursor-not-allowed shadow-md transition-all text-lg"
                        >
                            {isLoading ? <Spinner className="w-6 h-6 text-negro-fondo mx-auto" /> : 'Generar look de maquillaje'}
                        </button>
                    </Step>
                </div>

                <div className="bg-white/60 backdrop-blur-sm shadow-md p-4 rounded-lg flex flex-col min-h-[400px]">
                    <h3 className="text-lg font-bold text-center text-negro-fondo mb-4 border-b border-rosa-principal/20 pb-2">Resultados</h3>
                            ...
                            <div key={i} className="relative aspect-square bg-white/40 rounded-lg flex items-center justify-center">
                                {isLoading && i >= generatedImages.length ? (
                                    <div className="text-center">
                                        <Spinner className="w-8 h-8 text-negro-fondo mb-2" />
                                        <p className="text-[10px] text-negro-fondo opacity-60">Aplicando...</p>
                                    </div>
                                ) : generatedImages[i] ? (
                                ...
                                ) : <span className="opacity-30 text-3xl font-bold text-negro-fondo">{i + 1}</span>}
                            </div>
                        ))}
                    </div>
            ...
            <div className="space-y-6 mt-8">
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    ...
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isIntroExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[120px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsIntroExpanded(!isIntroExpanded)} className="mt-4 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
                ...
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    ...
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isHowItWorksExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[120px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsHowItWorksExpanded(!isHowItWorksExpanded)} className="mt-4 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
                ...
                <div className="bg-white/60 backdrop-blur-sm shadow-md p-6 rounded-lg">
                    ...
                    <div className={`text-negro-fondo opacity-90 space-y-4 leading-relaxed text-base transition-all duration-500 ease-in-out overflow-hidden relative ${isMarketingExpanded ? 'max-h-[70vh] overflow-y-auto' : 'max-h-[120px]'}`}>
                    ...
                    <div className="text-center">
                        <button onClick={() => setIsMarketingExpanded(!isMarketingExpanded)} className="mt-4 text-rosa-principal-dark font-bold hover:underline text-negro-fondo">
